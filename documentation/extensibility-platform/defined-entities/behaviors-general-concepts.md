...
Runtime Defined Entity Behaviors (or just Behaviors) are operations which execute custom logic in VMware Cloud Foundation Automation.
...
Behaviors can be executed on a defined entity via an API call if the user has the [necessary access permissions](#behaviors-access-control). In addition, behaviors can also be configured to be automatically executed in relation to the defined entity's lifecycle events (PreCreate, PostCreate, PostUpdate, PreDelete, PostDelete). Behaviors can also be [invoked statically](#staticstandalone-behavior-invocation) without referring to a defined entity.

Behaviors can be [scheduled for periodic execution](scheduled-behaviors.md) using cron expressions, enabling automated recurring operations.
...
API reference can be found [here](https://developer.broadcom.com/xapis/provider-infrastructure-apis/latest/cloudapi/1.0.0/interfaces/id/behaviors/post/).
...
API reference can be found [here](https://developer.broadcom.com/xapis/provider-infrastructure-apis/latest/cloudapi/1.0.0/entityTypes/id/behaviors/behaviorId/put/). 
...
Behavior invocation is an asynchronous operation in VMware Cloud Foundation Automation since a behavior execution is a long running process. For each behavior invocation, a `BEHAVIOR_INVOCATION` task is created to track the execution.
...
By default, behaviors as created as dynamic. In VMware Cloud Foundation Automation there is the option to define a behavior as static as well.
...
Headers:
Location: https://<vcfa-host>/api/task/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
```

API reference can be found [here](https://developer.broadcom.com/xapis/provider-infrastructure-apis/latest/cloudapi/1.0.0/entities/id/behaviors/behaviorId/invocations/post/).
...
API reference can be found [here](https://developer.broadcom.com/xapis/provider-infrastructure-apis/latest/cloudapi/1.0.0/interfaces/id/behaviors/behaviorId/invocations/post/).
...
Each behavior invocation is tracked by a `BEHAVIOR_INVOCATION` task. Once the behavior execution is completed, the tracking task will also be completed. If the behavior execution is successful, the task will succeed and the result of the execution will be in the `result` field of the task. If the behavior execution fails, the task will also fail with the appropriate error in the `error` field of the task.

### Behavior Execution Results

All behavior execution types return results as JSON-encoded strings in the task's `result.resultContent` field:

```json
{
  "result": {
    "resultContent": "{\"key\":\"value\"}",  // Always a JSON-encoded string
...
The execution result is always a JSON-encoded string (or null).
...
API reference can be found [here](https://developer.broadcom.com/xapis/provider-infrastructure-apis/latest/cloudapi/1.0.0/entityTypes/id/behaviorAccessControls/post/).
...
- [Pre Create](rde-hooks.md#pre-create-behavior-hook)
