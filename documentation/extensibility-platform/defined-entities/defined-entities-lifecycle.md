...
![Defined Entity Lifecycle](../../images/rde_lifecycle_with_prevalidated.png)
...
The PRE_VALIDATED state is an intermediate state for entities with PreCreate hooks defined.

When an entity type has a [PreCreate hook](rde-hooks.md#pre-create-behavior-hook) configured, newly created entities enter the PRE_VALIDATED state instead of PRE_CREATED. In this state:

- **Only deletion is allowed** - no update operations are permitted
- The entity awaits PreCreate hook execution completion
- On PreCreate hook success: entity transitions to PRE_CREATED state
- On PreCreate hook failure: entity is automatically deleted

This state ensures validation and potential content modification occur before the entity enters the standard creation phase.
