...
For example, an extension can define a type that has a property in its schema that describes the desired state of a system.
Users can create new instances of that type and fill in the property with their desired system state.

The extension can bind behaviors to the `OnCreate` and `OnUpdate` hooks of that type to observe the modifications in the desired state requested by the users.
The hook behaviors will be invoked upon such modifications and can initiate or perform processes to satisfy the modified desired state.
...
```text
POST
https://{{vcfa_host}}:{{vcfa_port}}/cloudapi/1.0.0/entityTypes
```
...
```json
{
    ...
    ...
    "hooks": {
        "PreCreate": "urn:vcloud:behavior-interface:validateCreate:clusterVendorA:containerCluster:1.0.0",
...
The type definition binds four of the interface behaviors to different lifecycle events of the entities of the type:

- the `validateCreate` behavior will be invoked before the entity is saved in the DB and will validate the entity contents to check whether they comply with some business-related prerequisite
...
- [Pre Create](#precreate-hook-behavior)
...
### PreCreate Hook Behavior

The PreCreate hook is invoked automatically on a defined entity instance immediately upon its creation, before it enters the `PRE_CREATED` state.

**Purpose**: Validate and optionally modify entity content before creation proceeds

**State Flow**:
- Entity created → Enters `PRE_VALIDATED` state
- PreCreate hook executes
- On success → Entity moves to `PRE_CREATED` state
- On failure → Entity is deleted

**Hook Response**:
- **Empty string ("")**: No modification, use original entity content
- **JSON string**: Replace entity content with the returned JSON
- **Error/Other**: Entity creation fails, entity is deleted

![RDE pre-create hook execution and RDE entity states](../../images/rde-pre-create-hook-entity-states.png)
...
```text
202 Accepted
Location: https://<vcfa-host>/api/task/<task-id>
```

The RDE create operation is a long-running process in VMware Cloud Foundation Automation which is traced by a task. If the RDE creation triggers a pre-create hook the task returned in the `Location` header of the create RDE API call response holds a reference to the PreCreate hook invocation task.

**Use Cases**:
- Validate entity content against business rules before creation
- Enrich entity content with computed/default values
- Enforce singleton patterns (allow only one entity of a type)
- Pre-populate fields based on external systems
...
The RDE create operation is a long-running process in VMware Cloud Foundation Automation which is traced by a task. If the RDE creation triggers a post-create hook the task returned in the `Location` header of the create RDE API call response holds a reference to the PostCreate hook execution task.
...
Headers:
...
X-VMWARE-VCLOUD-TASK-LOCATION: https://<vcfa-host>/api/task/<task-id>
...
```
...
If an entity is in an `IN_DELETION` entity state before a pre-delete hook execution, the hook is not executed. VMware Cloud Foundation Automation proceeds as if the pre-delete hook execution is successful (we assume entity can be deleted).
...
In the async scenario, the solution backend is expected to poll the VMware Cloud Foundation Automation API for entities which are marked for deletion.
...
in the request. This applies to all hook types including PreCreate, PostCreate, PreDelete, and PostDelete. For example:
