...
А new Runtime Defined Entity is [created as an instance of its RDE Type](https://developer.broadcom.com/xapis/provider-infrastructure-apis/latest/cloudapi/1.0.0/entityTypes/id/post/). For example, an entity of the type `urn:vcloud:type:clusterVendorA:basicContainerCluster:1.0.0` is created in the following way:

```text
POST https://{{vcfa_host}}:{{vcfa_port}}/cloudapi/1.0.0/entityTypes/urn:vcloud:type:clusterVendorA:basicContainerCluster:1.0.0
```
...
The entity contents do not have to be valid with respect to the JSON Schema of the entity's type until the entity is resolved.
...
```text
POST https://{{vcfa_host}}:{{vcfa_port}}/cloudapi/1.0.0/entityTypes/urn:vcloud:type:clusterVendorA:basicContainerCluster:1.0.0?resolveEntity=true
```
...
Runtime Defined Entities can be [retrieved by their IDs](https://developer.broadcom.com/xapis/provider-infrastructure-apis/latest/cloudapi/1.0.0/entities/id/get/) or [via queries](rde-queries.md).
...
```text
GET https://{{vcfa_host}}:{{vcfa_port}}/cloudapi/1.0.0/entities/urn:vcloud:entity:clusterVendorA:basicContainerCluster:646849d7-bc44-461c-96a4-6c297af0e753
```
...
```json
{
    ...
    "lastModificationDate": "2024-01-15T14:20:00.000Z",
    "owner": {
        "name": "companyAUser",
...
The `creationDate` and `lastModificationDate` fields are included in entity representations and can be used for querying and filtering.

## Entity Resolution

Once the entity contents have been fully filled in, [the entity can be resolved](https://developer.broadcom.com/xapis/provider-infrastructure-apis/latest/cloudapi/1.0.0/entities/id/resolve/post/).
For example:

```text
POST https://{{vcfa_host}}:{{vcfa_port}}/cloudapi/1.0.0/entities/urn:vcloud:entity:clusterVendorA:basicContainerCluster:646849d7-bc44-461c-96a4-6c297af0e753/resolve
```
...
Upon resolution, the entity contents will be validated against the entity type schema. If the validation is successful, the entity will transition to a `RESOLVED` state. Otherwise, it will transition to a `RESOLUTION_ERROR` state and the `message` property in the response will contain a description of the validation error.

## Entity Update

Runtime Defined Entities can be modified via the [RDE Update API request](https://developer.broadcom.com/xapis/provider-infrastructure-apis/latest/cloudapi/1.0.0/entities/id/put/).
...
```text
PUT https://{{vcfa_host}}:{{vcfa_port}}/cloudapi/1.0.0/entities/urn:vcloud:entity:clusterVendorA:basicContainerCluster:646849d7-bc44-461c-96a4-6c297af0e753
```
...
The RDE `owner` property is intended to identify the user whose quota would be used to account for the resources used by the Defined Entity. Its initial value is the user who created the entity.

Only users with FullControl access to the entity can update the `owner` property.

### Update the RDE `entityType` property

The RDE `entityType` property identifies the type of the entity and can only be changed to another version of the current type. See the [RDE Versioning](rde-versions.md#upgrading-an-rde-instance-to-another-type-version) section for details.
...
Runtime Defined Entities are deleted via the [DELETE API request](https://developer.broadcom.com/xapis/provider-infrastructure-apis/latest/cloudapi/1.0.0/entities/id/delete/).
...
## RDE Transfer Sessions

For entities involving file uploads/downloads (e.g., OVF templates, VM exports), VMware Cloud Foundation Automation manages transfer sessions.

### Upload Session Timeout

Transfer sessions include automatic timeout processing:

- Sessions that exceed their timeout are automatically marked
- Incomplete transfers move associated entities to `RESOLUTION_ERROR` state  
- Completed transfers before timeout keep entities in `RESOLVED` state
- Cleanup jobs monitor and process timed-out sessions

This ensures entities accurately reflect their transfer state and prevents orphaned transfer sessions.
