...
MQTT behaviors in VMware Cloud Foundation Automation are based on MQTT. Invocation of a MQTT behavior is a way to trigger a MQTT extension (send a message to the extension for processing). When invoked MQTT behaviors essentially post a message to an extension topic and listen for a response to that message.
...
```
POST https://<vcfa-host>/cloudapi/extensions/api/
```
```json
{
  "name": "testName",
...
- enabled - true/false (a user provided field) - whether the external service is enabled or not. If the external service is not enabled, mqtt behavior messages will not be sent to it.
- mqttTopics - (generated by VCFA per external service) - contains the MQTT topics which the external service will use to communicate with VCFA - the monitor topic is the one the external service will receive messages fromVCFA on, and the respond topic is the one the external service will use to send responses.

### Connection Reliability

The MQTT behavior framework properly handles partial MQTT handshake packets, improving reliability in unstable network conditions:
