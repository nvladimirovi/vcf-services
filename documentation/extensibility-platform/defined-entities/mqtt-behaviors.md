...
In order to create a MQTT behavior an MQTT API extension must be registered in VMware Cloud Foundation Automation first.

Please familiarize yourself with [MQTT API extensibility](../api-extensibility.md) and how to register such an extension.

### Connection Reliability

The MQTT behavior framework now properly handles partial MQTT handshake packets, improving reliability in unstable network conditions:

- Graceful handling of incomplete connection attempts
- Automatic retry logic for interrupted handshakes
- Better error reporting for connection failures

This ensures MQTT behaviors remain functional even with intermittent network issues.
...
### VMware Cloud Foundation Automation to Extension

Messages from VMware Cloud Foundation Automation to Extension are sent on the extension's monitor topic:
...
### Extension to VMware Cloud Foundation Automation

Response messages from VMware Cloud Foundation Automation to extension must be sent on the extension's respond topic:
...
There are two types of responses an extension can send back to VMware Cloud Foundation Automation - a simple response and a task update response.
...
Multiple task update responses can be sent back to VMware Cloud Foundation Automation. This allows the task progress to be updated continuously, for example. The last task update must complete the task. Once the task is completed, later task updates regarding this task are ignored.
