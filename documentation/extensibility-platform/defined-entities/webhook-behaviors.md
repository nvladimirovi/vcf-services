...
## WebHook Request Payload (VMware Cloud Foundation Automation -> WebHook Server)
...
## WebHook Response Payload (WebHook Server -> VMware Cloud Foundation Automation)

There are three possible response formats for the webHook server to send back to VMware Cloud Foundation Automation. Each one is processed differently by VMware Cloud Foundation Automation.
...
- Shared secret - this a string which is only known to VMware Cloud Foundation Automation and the webHook server (`_internal_key`). This shared secret will be used to generate the signature header
...
Each webHook request from VMware Cloud Foundation Automation can be verified by generating the signature using the shared secret and comparing it to the signature in the signature header from VMware Cloud Foundation Automation. If they match, the request is verified.
...
Slack has a functionality for creating incoming webHooks which can be used for posting messages to Slack channels. WebHook behaviors can be used to integrate Slack incoming webHooks with VMware Cloud Foundation Automation, allowing VCFA to post messages directly to a slack channel.
...
In order to use VMware Cloud Foundation Automation's webHook behaviors to send messages to slack webHooks, the following steps must be followed:
